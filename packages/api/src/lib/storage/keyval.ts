...
export function createStore(dbName: string, storeName: string): UseStore {
  ...
  return (txMode, callback) =>
    dbp.then((db) =>
      callback(db.transaction(storeName, txMode).objectStore(storeName), db),
    );
}

export type UseStore = <T>(
  txMode: IDBTransactionMode,
  callback: (store: IDBObjectStore, db: IDBDatabase) => T | PromiseLike<T>,
) => Promise<T>;
...
export function close(customStore = defaultGetStore()): Promise<void> {
  return customStore("readonly", (_, db) => Promise.resolve(db.close()));
}
 ...
 *
 * @param key
 * @param customStore Method to get a custom store. Use with caution (see the
 *   docs).
 */
...
/**
 * Set multiple values at once. This is faster than calling set() multiple
 * times. It's also atomic – if one of the pairs can't be added, none will be
 * added.
 *
 * @param entries Array of entries, where each entry is an array of `[key,
 *   value]`.
 * @param customStore Method to get a custom store. Use with caution (see the
 *   docs).
 */
...
/**
 * Update a value. This lets you see the old value and update it as an atomic
 * operation.
 *
 * @param key
