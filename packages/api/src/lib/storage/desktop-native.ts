...
import {
  ...
  type AppDatabaseNotificationSeverity,
...
export class NativeDesktopAppDatabase extends MemoryAppDatabase {
  override readonly kind: AppDatabaseKind = "sqlite-native";
  private opened = false;
  private searchIndexBatchDepth = 0;
  private searchIndexBatchDirty = false;
  ...
  override async open(): Promise<void> {
    ...
    const stateJson = await invokeNative<string | null>(
    ...
    if (stateJson) {
      this.fromState(JSON.parse(stateJson) as AppDatabaseState);
    }
    this.opened = true;
    await this.migrate();
    await this.replaceNativeSearchDocuments();
  }
  ...
  override async beginSearchIndexingBatch(): Promise<void> {
    this.searchIndexBatchDepth += 1;
  }

  override async endSearchIndexingBatch(): Promise<void> {
    if (this.searchIndexBatchDepth === 0) {
      return;
    }

    this.searchIndexBatchDepth -= 1;
    if (this.searchIndexBatchDepth === 0 && this.searchIndexBatchDirty) {
      this.searchIndexBatchDirty = false;
      await this.replaceNativeSearchDocuments();
      await this.persist();
    }
  }
  ...
  override async configureSearchEmbeddingProvider(
    ...
    await super.configureSearchEmbeddingProvider(provider as never);
    await this.persist();
  }

  override async saveMetadataSnapshot(snapshot: unknown): Promise<void> {
    await super.saveMetadataSnapshot(snapshot as never);
    await this.persist();
  }

  override async upsertIndexedFile(
    record: Parameters<MemoryAppDatabase["upsertIndexedFile"]>[0],
  ): Promise<void> {
  ...
  override async deleteIndexedFile(path: string): Promise<void> {
    ...
    await this.persist();
    await this.deleteNativeSearchDocument(path);
  }
  ...
  override async upsertSearchDocument(
    document: Parameters<MemoryAppDatabase["upsertSearchDocument"]>[0],
  ): Promise<void> {
    await super.upsertSearchDocument(document);
    if (this.searchIndexBatchDepth > 0) {
      this.searchIndexBatchDirty = true;
      return;
    }
    const persistedDocument = this.searchDocs.get(document.path);
    if (persistedDocument) {
      await this.upsertNativeSearchDocument(persistedDocument);
    }
    await this.persist();
  }
  ...
  override async searchDocuments(
    ...
    if (
      ...
      (options.mode === "vector" ||
        options.mode === "hybrid" ||
        (options.mode !== "lexical" && this.searchEmbeddingProvider))
    ) {
      return this.searchDocumentsWithNativeVectors(query, options);
    }
    ...
    try {
      ...
      const candidates = await invokeNative<Array<{ path: string }>>(
        "desktop_db_search_documents",
        { vaultId: this.vaultId, terms, limit },
      );
      const source = candidates
        .map((candidate) => this.searchDocs.get(candidate.path))
      ...
      const results = source
        ...
        .filter(
          (result) =>
            (result.scoreBreakdown.lexical ?? 0) > 0 &&
  ...
  private async searchDocumentsWithNativeVectors(
    query: string,
    options: AppDatabaseSearchOptions,
  ): Promise<AppDatabaseSearchResult[]> {
    ...
    const limit = options.limit ?? 100;
    let nativeVectorResult: NativeVectorSearchResult;
    try {
      nativeVectorResult = await invokeNative<NativeVectorSearchResult>(
        "desktop_db_search_vector_documents",
        { vaultId: this.vaultId, queryVector, limit },
      );
    } catch {
      return super.searchDocuments(query, options);
    }

    if (!nativeVectorResult.available) {
      return super.searchDocuments(query, options);
    }

    const vectorScores = new Map(
      nativeVectorResult.candidates.map((candidate) => [
        candidate.path,
        {
          score: candidate.score,
          matchedChunkIds: candidate.matchedChunkIds,
        },
      ]),
    );
    const vectorCandidatePaths = new Set(vectorScores.keys());
      ...
      if (terms.length) {
        try {
          ...
          for (const candidate of candidates) {
            lexicalCandidates.add(candidate.path);
          }
        } catch {
          // Keep vector candidates and let the shared evaluator score them.
        }
      }
    }
    ...
    const source = [...candidatePaths]
      .map((path) => this.searchDocs.get(path))
      .filter((document): document is SearchDocumentRecord =>
        Boolean(document),
      );

    if (!source.length && candidatePaths.size > 0) {
      return [];
    }
    if (!source.length) {
      return super.searchDocuments(query, options);
    ...
    const vectorCandidateCount = vectorCandidatePaths.size;
    const appliedMode =
    ...
    const resolvedVectorScores = new Map(
      source.map((document) => {
        const nativeVectorScore = vectorScores.get(document.path);
        const fallbackVectorScore = nativeVectorScore
          ? null
          : scoreVectorDocument(document, queryVector);
        return [
          document.path,
          nativeVectorScore ??
            fallbackVectorScore ?? { score: 0, matchedChunkIds: [] },
        ];
      }),
    );
    const lexicalRanks = rankSearchScores(
    ...
    const vectorRanks = rankSearchScores(
      [...resolvedVectorScores.entries()].map(([path, entry]) => ({
        path,
        score: entry.score,
      })),
    ...
    const results = source
      .map((document) => {
        const vectorScore = resolvedVectorScores.get(document.path) ?? {
          score: 0,
          matchedChunkIds: [],
        };

        return buildSearchResult(document, query, options, {
          backendKind: this.kind,
          appliedMode,
          lexicalScore: lexicalScores.get(document.path) ?? 0,
          vectorScore: vectorScore.score,
          lexicalCandidateCount: lexicalCandidates.size,
          vectorCandidateCount,
          providerConfig: this.searchEmbeddingProviderConfig,
          preferredChunkIds: vectorScore.matchedChunkIds,
          lexicalRank: lexicalRanks.get(document.path),
  ...
  private async replaceNativeSearchDocuments(): Promise<void> {
    await invokeNative<void>("desktop_db_replace_search_documents", {
      vaultId: this.vaultId,
      documents: [...this.searchDocs.values()],
    });
  }

  private async upsertNativeSearchDocument(
    document: SearchDocumentRecord,
  ): Promise<void> {
    await invokeNative<void>("desktop_db_upsert_search_document", {
      vaultId: this.vaultId,
      document,
    });
  }

  private async deleteNativeSearchDocument(path: string): Promise<void> {
    await invokeNative<void>("desktop_db_delete_search_document", {
      vaultId: this.vaultId,
      path,
    });
  }
  ...
  private async persist(): Promise<void> {
    ...
    await invokeNative<void>("desktop_db_save_state", {
      vaultId: this.vaultId,
      stateJson: JSON.stringify(this.toState()),
    });
  }
}
