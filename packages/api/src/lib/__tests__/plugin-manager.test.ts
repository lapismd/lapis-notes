...
let workspaceDependencies: Record<string, any>;
let createWorkspacePluginDependencyResolver: PluginDependencyResolverFactory;

const REPO_ROOT = resolve(process.cwd(), "../..");

const EXTERNAL_PLUGIN_INSTALL_BUNDLE_ROOTS = [
  "packages/plugins/plugin-canvas",
  "packages/plugins/plugin-docs",
  "packages/plugins/plugin-graph",
  "packages/plugins/plugin-markdown-lint",
  "packages/plugins/plugin-pdf",
  "packages/plugins/plugin-slides",
  "packages/plugins/plugin-telemetry",
  "packages/notebook",
] as const;

type InstallBundleFixture = {
  pluginId: string;
  distDir: string;
  manifest: Record<string, unknown>;
  entryFilePath: string;
  entryFormat: "commonjs" | "esm";
  entryPath: string;
};

const EXTERNAL_PLUGIN_INSTALL_BUNDLES: InstallBundleFixture[] =
  EXTERNAL_PLUGIN_INSTALL_BUNDLE_ROOTS.map((pluginRoot) => {
    const rootPath = join(REPO_ROOT, ...pluginRoot.split("/"));
    const manifestCandidates = [
      join(rootPath, "src", "manifest.json"),
      join(rootPath, "manifest.json"),
      join(rootPath, "dist", "manifest.json"),
    ];
    const manifestPath = manifestCandidates.find((candidate) =>
      existsSync(candidate),
    );
    if (!manifestPath) {
      throw new Error(`No manifest found for install bundle: ${pluginRoot}`);
    }
    ...
    if (!pluginId) {
      throw new Error(`Install bundle manifest missing id: ${manifestPath}`);
    }
    const distDir = join(rootPath, "dist");
    const runtimeEntry = getWorkspaceRuntimeEntry(manifest);
    const entryPath = runtimeEntry?.path ?? "main.js";
    const entryFormat = runtimeEntry?.format ?? "commonjs";
    return {
      pluginId,
      distDir,
      manifest,
      entryFilePath: join(distDir, entryPath),
      entryFormat,
      entryPath,
    };
  });

function collectBundleSourceFiles(directory: string): Record<string, string> {
  const files: Record<string, string> = {};

  const visit = (folderPath: string, basePath: string = "") => {
...
function getWorkspaceRuntimeEntry(
  manifest: Record<string, unknown>,
): { format?: "commonjs" | "esm"; path?: string } | null {
  const lapis = manifest["lapis"];
  if (!lapis || typeof lapis !== "object") {
    return null;
  }
  const runtime = (lapis as Record<string, unknown>)["runtime"];
  if (!runtime || typeof runtime !== "object") {
    return null;
  }
  const entries = (runtime as Record<string, unknown>)["entries"];
  if (!entries || typeof entries !== "object") {
    return null;
  }
  const workspace = (entries as Record<string, unknown>)["workspace"];
  if (typeof workspace === "string") {
    return { format: "commonjs", path: workspace };
  }
  ...
  const format =
    entry["format"] === "esm" || entry["format"] === "commonjs"
      ? entry["format"]
      : undefined;
  return { format, path };
}

function getBareRequiresFromEntry(entryFilePath: string): string[] {
  return getReachableBareRequiresFromEntry(
    dirname(entryFilePath),
    entryFilePath.split("/").pop() ?? "main.js",
  );
}

function createInstallBundleDependencies(): Record<string, any> {
  const apiModule = {
    mountComponent: () => ({
      destroy() {},
      props: {},
  ...
  const svelteModule = {
    ...workspaceDependencies.svelte,
    mount: () => ({ $destroy() {}, destroy() {} }),
    unmount: () => {},
  };
  ...
  return {
    ...
    obsidian: {
      ...workspaceDependencies.obsidian,
      mountComponent: apiModule.mountComponent,
    },
    "@lapis-notes/api": apiModule,
    svelte: svelteModule,
  };
}

function createInstallBundlePluginManagerOptions(): PluginManagerOptions {
  return {
    createCommunityPluginDependencyResolver: (deps) =>
      createWorkspacePluginDependencyResolver({
        ...createInstallBundleDependencies(),
        ...deps,
      }),
  };
}
...
function resolveBundleLocalRequire(
  distDir: string,
  importerPath: string,
  specifier: string,
): string | null {
  const basePath = join(dirname(join(distDir, importerPath)), specifier);
  const candidates = [
    basePath,
    `${basePath}.js`,
    `${basePath}.cjs`,
  ...
  return resolved ? resolved.slice(distDir.length + 1) : null;
}

function getReachableBareRequiresFromEntry(
  distDir: string,
  entryPath: string,
): string[] {
  ...
  while (pending.length > 0) {
    ...
    for (const specifier of parseRequireSpecifiers(
      readFileSync(join(distDir, filePath), "utf8"),
    )) {
      if (specifier.startsWith(".") || specifier.startsWith("/")) {
        const resolved = resolveBundleLocalRequire(
          distDir,
          filePath,
          specifier,
        );
...
async function seedCommunityPluginFromInstallBundle(
  app: App,
  fixture: InstallBundleFixture,
) {
  const pluginPath = `/.obsidian/plugins/${fixture.pluginId}`;
  await app.vault.mkpath(pluginPath);
  await app.vault.create(
    `${pluginPath}/manifest.json`,
    JSON.stringify(fixture.manifest, null, 2),
  );
  const files = collectBundleSourceFiles(fixture.distDir);
  for (const [relativePath, source] of Object.entries(files)) {
    const parentPath = relativePath.split("/").slice(0, -1).join("/");
    if (parentPath) {
      await app.vault.mkpath(`${pluginPath}/${parentPath}`);
    }
    await app.vault.create(`${pluginPath}/${relativePath}`, source);
  }
}
...
function ensureInstallBundleArtifacts(): void {
  const missingRoots = EXTERNAL_PLUGIN_INSTALL_BUNDLE_ROOTS.filter((root) => {
    const fixture = EXTERNAL_PLUGIN_INSTALL_BUNDLES.find((candidate) =>
      candidate.distDir.startsWith(join(REPO_ROOT, root)),
    );
    return !fixture || !existsSync(fixture.entryFilePath);
  });
  if (missingRoots.length === 0) {
    return;
  }

  const filters = missingRoots.map((root) => {
    const packageJsonPath = join(REPO_ROOT, root, "package.json");
    const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
      name?: string;
    };
    if (!packageJson.name) {
      throw new Error(
        `Install bundle package missing name: ${packageJsonPath}`,
      );
    }
  ...
  if (result.status !== 0) {
    throw new Error(
      `Failed to build install-bundle plugin artifacts for: ${missingRoots.join(", ")}`,
    );
  }

  const stillMissing = EXTERNAL_PLUGIN_INSTALL_BUNDLE_ROOTS.filter((root) => {
    const fixture = EXTERNAL_PLUGIN_INSTALL_BUNDLES.find((candidate) =>
      candidate.distDir.startsWith(join(REPO_ROOT, root)),
    );
    return !fixture || !existsSync(fixture.entryFilePath);
  ...
  if (stillMissing.length > 0) {
    throw new Error(
      `Install-bundle plugin runtime entry still missing after build: ${stillMissing.join(", ")}`,
    );
  }
}
...
describe("PluginManager", () => {
  beforeAll(async () => {
    ensureInstallBundleArtifacts();
    workspaceDependencies = (await import("../../../../workspace/src/lib/deps"))
      .default;
    createWorkspacePluginDependencyResolver = (
      await import(
        "../../../../workspace/src/lib/plugin-runtime/workspace-plugin-dependency-resolver"
      )
    ).createWorkspacePluginDependencyResolver;
  }, 120_000);
  ...
  it("keeps official external install-bundle artifacts ESM-only", () => {
    expect(
      EXTERNAL_PLUGIN_INSTALL_BUNDLES.map((fixture) => ({
        pluginId: fixture.pluginId,
        entryFormat: fixture.entryFormat,
        entryPath: fixture.entryPath,
        hasEntry: existsSync(fixture.entryFilePath),
        hasLegacyMainJs: existsSync(join(fixture.distDir, "main.js")),
      })),
    ).toEqual(
      EXTERNAL_PLUGIN_INSTALL_BUNDLES.map((fixture) => ({
        pluginId: fixture.pluginId,
        ...
        entryPath: "main.mjs",
        hasEntry: true,
        hasLegacyMainJs: false,
      })),
    );
  });

  it.each(
    EXTERNAL_PLUGIN_INSTALL_BUNDLES.filter(
      (fixture) => fixture.entryFormat === "commonjs",
    ),
  )(
    "loads external install-bundle plugin $pluginId from dist artifacts",
    async (fixture) => {
      const { app } = createTestApp(createInstallBundlePluginManagerOptions());
      await app.vault.load();
      app.plugins.registerDependencies(createInstallBundleDependencies());
      await seedCommunityPluginFromInstallBundle(app, fixture);
      await app.vault.create(
        "/.obsidian/community-plugins.json",
        JSON.stringify([fixture.pluginId], null, 2),
      );
      ...
      expect(diagnostics?.lastFailureMessage ?? "").not.toContain(
        "Cannot require",
      );
      expect(app.plugins.plugins.get(fixture.pluginId)?.enabled).toBe(true);
    },
    30_000,
  );
  ...
  it("rejects unknown bare dependencies during install-bundle evaluation", async () => {
    const { app } = createTestApp();
    await app.vault.load();
    app.plugins.registerDependencies(workspaceDependencies);
    await seedCommunityPlugin(
      app,
      "unknown-bare-bundle",
      `const _ = require("@lapis-notes/api/does-not-exist");
module.exports = class UnknownBareDependencyPlugin extends require('obsidian').Plugin {
  onload() {}
}`,
    );
    await app.vault.create(
      "/.obsidian/community-plugins.json",
      JSON.stringify(["unknown-bare-bundle"], null, 2),
    );

    await app.plugins.loadPlugins();

    expect(app.plugins.plugins.get("unknown-bare-bundle")?.enabled).not.toBe(
      true,
    );
    expect(
      app.plugins.getCommunityPluginDiagnostics("unknown-bare-bundle")?.state,
    ).toBe("failed");
    expect(
      app.plugins.getCommunityPluginDiagnostics("unknown-bare-bundle")
        ?.lastFailureMessage,
    ).toContain(
      "Cannot require @lapis-notes/api/does-not-exist in plugin unknown-bare-bundle",
    );
  });

  it("verifies external install-bundle bare requires are pre-registered in deps", () => {
    const dependencyResolver = createWorkspacePluginDependencyResolver(
      createInstallBundleDependencies(),
    );
    const missing = EXTERNAL_PLUGIN_INSTALL_BUNDLES.flatMap((fixture) => {
      if (fixture.entryFormat !== "commonjs") {
        return [];
      }
      return getBareRequiresFromEntry(fixture.entryFilePath)
        .filter((specifier) => {
          const context: PluginDependencyContext = {
            pluginId: fixture.pluginId,
            pluginPath: `/.obsidian/plugins/${fixture.pluginId}`,
            manifest:
              fixture.manifest as unknown as PluginDependencyContext["manifest"],
            host: "workspace",
            format: "commonjs",
          };
          return !dependencyResolver.canResolve(specifier, context);
        })
        .map(
          (specifier) =>
            `${fixture.pluginId} requires ${specifier} which is missing from dependency registry`,
        );
    });
