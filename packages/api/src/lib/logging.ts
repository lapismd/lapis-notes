 ...
 */
export class LogManager extends EventDispatcher<LogEvents> {
  ...
  public setLevel(module: string, level: TLogLevelName) {
    this.options.minLevels[module] = level;
  }
  ...
  public registerConsoleLogger(
    props: Partial<{ color: boolean }> = { color: true },
  ): LogManager {
    ...
    this.onLogEntry((logEntry) => {
      ...
      if (props.color) {
        ...
        const colors: Record<string, string> = {
          trace: "color: #9c27b0; font-weight: italic",
          debug: "color: #666666; font-style: italic",
          info: "color: green;",
          warn: "color: #ffc107; font-weight: bold",
          error: "color: #e74c3c; font-weight: bold",
        };
        args.push(colors[logEntry.level]);
      }

      args.push(logEntry.objects);
      ...
      switch (logEntry.level) {
        ...
        case "error":
          ...
          break;
        default:
          console.log(`{${logEntry.level}}`, ...args);
      }
    });
 ...
 */
export class Logger {
  private logManager: EventDispatcher<LogEvents>;
   ...
   */
  constructor(
    logManager: EventDispatcher<LogEvents>,
    module: string,
    readonly getLevel: () => string,
  ) {
  ...
  get minLevel(): number {
    return this.levelToInt(this.getLevel());
  }
  ...
  /**
   * Central logging method with a trace ID to track calls between
   * modules/components.
   *
   * @param logLevel
   * @param message
...
/**
 * This deceleration will log the time taken to run the function it is attached
 * to. Be careful where it is added as it increases the output.
 *
 * @returns {any}
 */
...
/**
 * Provides a simple log function that can be used to log messages against
 * default module.
 *
...
/**
 * This allows the plugin to be debugged in a mobile application add it when
 * debugging on a device. Not meant to be used by end users. Add it into main.ts
 * and remove before you commit.
 *
 * @param {Plugin} plugin
 * @returns {any}
 */
