...
| Backlinks / Outgoing Links                         | Done           | Full-width titleless 0.75rem result surfaces with one trailing count edge; tested linked/unlinked mention model, grouping, search, sort, context, collapse, navigation, refresh, and shared app-bound Mira `FileEmbed` previews inside Design Core Hover Card with owner-document portals, native collision handling, topmost cross-pane paint, interactive safe pointer handoff, and pinned editable full-note previews. Ordinary document links retain the writable Lapis adapter while Mira owns their portaled Bits UI preview and editable lifecycle. |
| Portable embed framework                           | Done           | Public root and `./embed` `FileEmbed`, `MarkdownEmbed`, and `NoteLink`; additive `FileEmbed` editable/editing inputs; Lapis `MiraFileAdapter` supplies vault-relative resolution, Markdown refresh and writes, navigation, image/custom embed lifecycle, file/headings catalogs, and shared reading/live-preview behavior. Direct embeds remain read-only unless their owner explicitly enables editable hover-preview use.                                                                                                                                |
