...
| LN-DENO-067 | The desktop development renderer MUST keep `@lapis-notes/api` outside dependency optimization and resolve it as a singleton across the host and packed static plugins. Workspace host bindings and other API-owned state MUST remain visible when a plugin is loaded through a local release-candidate tarball. |
