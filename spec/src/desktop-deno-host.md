...
| LN-DENO-067 | The desktop renderer MUST resolve `@lapis-notes/api` as a singleton across the host and packed plugins. Before evaluating a verified ESM installed plugin, it MUST expose host-owned API and Markdown embed namespaces plus the compiler-emitted `svelte`, `svelte/internal/client`, and `svelte/internal/disclose-version` ABI through import-map facades. Registry releases and local candidates MUST bind the same API and Svelte peers so workspace state and component lifecycles remain visible. |
