...
| LN-ED-023 | Focused Backlinks and Outgoing Links acceptance MUST open the hover preview and verify that it resolves to the panel-specific 26rem default width, contains the public app-bound `FileEmbed`, Mira's `internal-embed` host, and an embedded Markdown surface rather than only asserting generic popover visibility or duplicated private preview copy. |
