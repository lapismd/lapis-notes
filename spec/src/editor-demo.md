...
| LN-ED-022 | The API editor's design-core `ScrollArea` root MUST fill and remain bounded by its owning `WorkspaceViewHost`, leaving the ScrollArea viewport as the vertical scroll owner when a source or Markdown document is taller than its pane. Focused Storybook acceptance MUST prove a long Markdown document has a usable scroll range and accepts a changed scroll position in both top-tab and stacked-tab workspace placements. |
| LN-ED-023 | Focused Backlinks and Outgoing Links acceptance MUST open the hover preview and verify that it contains the public app-bound `FileEmbed`, Mira's `internal-embed` host, and an embedded Markdown surface rather than only asserting generic popover visibility or duplicated private preview copy. |
