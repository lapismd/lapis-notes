...
| LN-ED-007 | The demo MUST adapt Design Core Explorer to the API vault for listing, active-file selection, create, open, rename, move, delete, persisted auto-reveal, and persisted show-hidden files. Dotted names MUST stay hidden unless that setting is on. The left-sidebar Explorer play MUST persist the toolbar toggle to `.obsidian/app.json` and reveal `.env` and `.obsidian`. |
| LN-ED-008 | The empty landing view MUST expose functional Create note and Go to file actions. Go to file MUST open the Design Core palette on the Files tab with its bounded starter files. Close MUST use the design-core leaf action. |
...
| LN-ED-041 | Focused Markdown Authoring acceptance MUST prove borderless Reading paint, inherited Design Core preview scrolling, View-first menu order, toolbar toggling, and persisted toolbar-driven editor settings. |
...
| LN-ED-044 | Storybook and production desktop hosts MUST consume the same built `@lapis-notes/file-explorer` public package boundary, and publishable consumers MUST use its npm semver range. Tags MUST come from `@lapis-notes/markdown`. Storybook MAY parameterize Explorer loading state, but MUST NOT retain duplicate plugin or view implementations. |
...
fixture.
Markdown `notes_list` and API file tools reuse the same Vault contract but
