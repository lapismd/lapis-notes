...
| LN-WEB-004 | The web session MUST register File Explorer, Markdown, Markdown Lint, Spell Check, Search, Bookmarks, History, Word Count, Bases, and AI as optional bundled plugins plus enabled-by-default external Roles, Terminal, and Tasks. It MUST load only their configured enabled set before metadata and layout restoration, then mount the shared `WorkspaceShell`.            |
