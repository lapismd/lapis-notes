...
| LN-WEB-004 | The web session MUST register File Explorer, Markdown, Markdown Lint, Spell Check, Search, Graph, Bookmarks, History, Word Count, Bases, and AI as optional bundled plugins plus enabled-by-default external Roles, Terminal, and Tasks. Graph MUST follow Search and precede Bookmarks. The host MUST load only the configured enabled set before metadata and layout restoration, then mount the shared `WorkspaceShell`. |
...
| LN-WEB-036 | Web Vite MUST allow the development `@lapis-notes/lapis-plugin-terminal` package root through `server.fs.allow` so packaged `ghostty-vt.wasm` is served in development until that plugin is published.                                                                                                                                                                                                                    |
