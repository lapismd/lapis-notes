...
| LN-EXP-004 | The `lapis-vault-files` provider MUST declare the Files tab. An empty query MUST list workspace recents under Recent or, when a fresh vault has no visible recents, a bounded deterministic starter list of visible files. Queries MUST keep path filtering and the show-hidden setting. Landing Go to file MUST open the Files tab. |
...
The Files palette tab lists workspace recents under Recent when the query is
empty. A fresh vault with no visible recents returns a capped path-sorted starter
list so Go to file is not blank on first launch. A typed query keeps today's
path filter. Landing Go to file opens that tab through `app:go-to-file`.
