...
| LN-GRAPH-009 | The renderer MUST preserve force layout, pan, zoom, drag, keyboard focus, hover, labels, arrows, fit, and resize alignment. Zoom-out and fit MUST allow the complete settled graph bounds to appear within the viewport. Renderer colors and empty-state paint MUST resolve from `--ui-graph-*` tokens. Settings MUST persist through plugin data.                                                       |
