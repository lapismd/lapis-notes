...
| LN-ARCH-069 | Deno native close, menu, and external-navigation policy MUST remain host-owned while session persistence and disposal remain renderer-owned. On macOS the adopted bootstrap and visible window MUST route close events through one coordinator. The host MUST notify the renderer through a private same-origin signal without mutating the secondary native window after close begins. Renderer acknowledgement MUST cross the bounded native binding and fail closed on timeout without bypassing shared workspace or plugin teardown. |
