...
| LN-ARCH-077 | Graph canvas simulation and camera state MUST remain renderer-owned and independent from canonical data persistence. Topology selects entrance versus gentle reheat; display-only changes preserve positions. Viewport culling MUST be a paint optimization only and MUST NOT mutate the force model, navigation targets, or hit-test population. |
