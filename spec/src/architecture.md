...
production hosts continue to load that plugin. Hosts register the Tasks package
...
| LN-ARCH-087 | Public package release workflows MUST remain distribution infrastructure for `@lapis-notes/ui`, `@lapis-notes/api`, and `@lapis-notes/workspace`. They MUST NOT move host boot, plugin registration, vault selection, persistence, desktop native bindings, web routing, or bundled plugin policy into those public packages. |
