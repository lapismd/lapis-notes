...
| LN-ARCH-055 | AI conversation files inside their owning vault folder MUST be the portable source of truth and use API-owned generic vault durability. Chat and folder-aware history MUST remain separate sidebar views joined by a non-authoritative scope-and-ID reopen hint, and live vault folder moves MUST update that hint in memory. The provider-filtered AppDatabase projection MUST be disposable, rebuildable, and used only for cross-scope discovery; host replay buffers and native agent state MUST likewise remain non-canonical. |
