...
| LN-ARCH-055 | AI conversation files inside their owning vault folder MUST be the portable source of truth and use API-owned generic vault durability. View state MAY retain only a non-authoritative scope-and-ID reopen hint and MUST follow live vault folder moves in memory. AppDatabase projections, host replay buffers, and native agent state MUST remain independently disposable and MUST NOT be required to render local history. |
