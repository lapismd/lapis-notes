...
| LN-DESK-055 | Electron MUST advertise `terminal-runtime` and handle `desktop_terminal_session_*` IPC through sibling `@lapismd/terminal-host` in-process. The renderer MUST NOT import `node-pty`. |
| LN-DESK-056 | Desktop Vite MUST treat `*.wasm` as assets so the Terminal plugin Ghostty file resolves through `import.meta.url`. |
| LN-DESK-057 | `setNativeDesktopBridge` MUST publish or clear `globalThis.__LAPIS_NATIVE_DESKTOP__` so every API copy sees the same bridge. |
