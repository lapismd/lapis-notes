...
| LN-DESK-027 | Before restoring a vault layout, the desktop host MUST register Markdown, Markdown Lint, File Explorer, and Search as optional bundled plugins enabled by default. It MUST load only their configured enabled set before metadata and layout restoration while keeping community plugins disabled. |
