...
| LN-DESK-008 | The bridge MUST advertise resource, database, search, language-service, plugin-sidecar, plugin-assets, file-watch, notifications, file-system-actions, agent-runtime, and terminal-runtime capabilities as available. Notebook and model capabilities MUST remain unavailable.                                                                                                                                                                   |
...
| LN-DESK-019 | A native vault without `.obsidian/workspace.json` MUST show one empty `New Tab`, the left dock open at `22rem` with File Explorer, Search, then Bookmarks when those views are registered, and the right dock open with Outline, File Properties, then Tags when Markdown is enabled. The bottom dock MUST stay closed. Startup MUST NOT write a layout file or inject fixture views.                                                            |
...
| LN-DESK-027 | Before restoring a vault layout, the desktop host MUST register Markdown, Markdown Lint, Spell Check, File Explorer, Search, Bookmarks, History, Word Count, and Bases as optional bundled plugins enabled by default. It MUST load only their configured enabled set before metadata and layout restoration while keeping community plugins disabled.                                                                                           |
...
| LN-DESK-055 | Electron MUST advertise `terminal-runtime` and handle `desktop_terminal_session_*` IPC through sibling `@lapismd/terminal-host` in-process. Production preparation MUST copy the linked `node-pty` native prebuilds beside the bundled main process. The renderer MUST NOT import `node-pty`.                                                                                                                                                    |
| LN-DESK-056 | Desktop Vite MUST treat `*.wasm` as assets so the Terminal plugin Ghostty file resolves through `import.meta.url`.                                                                                                                                                                                                                                                                                                                               |
| LN-DESK-057 | `setNativeDesktopBridge` MUST register the bridge for every API copy and publish or clear the writable compatibility alias. A read-only contextBridge preload alias MUST remain authoritative and MUST NOT be overwritten.                                                                                                                                                                                                                       |
| LN-DESK-058 | Desktop Vite MUST allow the linked `@lapis-notes/lapis-plugin-terminal` package root through `server.fs.allow` so packaged `ghostty-vt.wasm` is served in development.                                                                                                                                                                                                                                                                           |
| LN-DESK-059 | Electron MUST bind each Terminal session workspace to the create payload vault path, otherwise `terminal.cwd`, otherwise the home directory.                                                                                                                                                                                                                                                                                                     |
...
`@lapis-notes/language-service/markdownlint/runtime`, which publishes
vscode-markdownlint Fix this / Fix all / Disable line / file / vault titles
for the current rule path. That runtime applies vault
`markdown-lint.disabledRules`, which seed MD013 off to match
vscode-markdownlint, and formats each diagnostic as the rule-name path plus
description used by vscode-markdownlint. The desktop e2e Markdown protocol
asserts those action titles.
