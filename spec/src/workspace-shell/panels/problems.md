...
Problems collections. `registerIndexProjection` does not add Problems
diagnostics or language-service apply paths.
...
| LN-WS-033 | `LanguageServiceManager` MUST publish completed open-document diagnostics into one shared collection and cache matching code actions. For concurrent requests on one resource, only the newest request MAY publish diagnostics or cached actions. It MUST reference-count editor ownership, preserve results across a same-turn editor remount, clear a true final-close and provider-unload result, and MUST NOT scan unopened vault resources. |
...
| LN-WS-036 | The enabled Markdownlint static plugin MUST own the sole renderer provider, consume the public `@lapis-notes/language-service` package through built npm exports, prefer the probed native Markdown capability, and fall back to its worker. The reusable language-service client MUST construct that fallback from its package-stable emitted JavaScript URL as a module worker, without a Vite query-suffix import. It MUST preserve `markdown-lint.disabledRules`, fixes, and ignore actions through the existing provider contract. The desktop host MUST NOT register a competing provider. |
