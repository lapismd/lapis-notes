...
| LN-DB-040 | AppDatabase MUST expose typed memory source, candidate, origin, recall-signal, and job records through Memory and Turso providers plus browser and native allowlisted bridges. These records MUST remain derived state and MUST NOT become transcript or curated-memory authority. |
| LN-DB-041 | Episodic and curated memory Search documents MUST use distinct provider ids and scope-bearing synthetic paths. Provider and scope filters MUST apply before candidate limits, while embedding identity changes MUST retain the existing disposable reindex behavior. |
| LN-DB-042 | Memory recall signals MUST store only a per-vault salted query fingerprint, day, bounded score, and count. Raw queries MUST NOT enter memory state, telemetry, job errors, or review artifacts. |
| LN-DB-043 | Memory jobs MUST support one renewable owner lease, bounded attempts, resumable cursors, and post-commit `memory` change notifications. Losing a lease or failing a job MUST leave published Search rows and durable vault files intact. |
| LN-DB-044 | The memory schema migration MUST be additive and transactional. Deleting all derived memory tables and Search rows MUST permit a deterministic rebuild from transcripts, curated Markdown, revision files, reviews, and exclusions without touching unrelated AppDatabase domains. |
