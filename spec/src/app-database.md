...
| LN-DB-003 | The default local provider MUST use pinned Turso native storage where supported and Turso WASM with OPFS in browser-compatible runtimes. |
...
| LN-DB-009 | A legacy SQLite database MUST be imported transactionally once, identified by a persisted source fingerprint, and retained unchanged after success or failure. |
...
| LN-DB-012 | Missing OPFS, isolation, locks, or channels MUST produce an explicit blocked or degraded state with accurate capabilities rather than risking concurrent database writers. |
...
The provider persists generated state outside the user-visible vault. Memory
and IndexedDB providers are test or degraded-runtime fallbacks, not the normal
desktop or isolated-browser path.
