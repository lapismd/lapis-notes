...
| LN-BASE-013 | Table, cards, and list presentation MUST retain the reference geometry and control density at the Bases boundary while using current Design Core theme tokens and accessible contrast. Table headers, rows, and summaries MUST share one ordered width model and remain aligned before, during, and after column resizing and horizontal scrolling. |
