...
| LN-CV-029 | The form-area header MUST show one book-or-pencil action when Markdown is selected. In the CV form area it MUST precede the RenderCV theme control. It MUST switch between Preview and read-only Source, default to Preview, preserve the compiled value, and keep its focus indicator inside the header. Both surfaces MUST fill their zoomed document width without additional outer inset, nested toolbar, border, radius, or shadow chrome. |
