...
| LN-UI-009 | Root `pnpm check:no-tailwind` (and package `check` wiring) MUST fail when LN-UI-008 is violated under `packages/ui/src/lib/components` and `packages/api/src/lib/components` (stories/examples excluded). |
