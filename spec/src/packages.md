...
| LN-PKG-004 | The web host, notebook, and unlisted plugins MUST NOT be added until their requirements are specified and tracked in `MIGRATION.md`. The Electron host, Markdown packages, File Explorer, workspace integration, and internal language service are the current authorized exceptions. |
...
| LN-PKG-044 | `@lapis-notes/search` MUST own Search indexing policy, workspace contribution, and result presentation. It MUST consume API query and database contracts plus Design Core filter presentation without moving vault search policy into `@lapis-notes/workspace`. |
