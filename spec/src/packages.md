...
| LN-PKG-004 | Notebook and remaining unlisted plugin packages MUST NOT be added until specified and tracked in `MIGRATION.md`. The authorized hosts are Electron and web. Authorized plugins are Markdown, Markdownlint, File Explorer, Search, Bases, and Roles. |
...
| LN-PKG-054 | `@lapis-notes/bases` MUST live at `packages/plugins/plugin-bases`, expose build, check, test, publint, root runtime, and stylesheet contracts, and publish only its governed public surface. It MUST NOT depend on the pruned `@lapis-notes/ui` package. |
