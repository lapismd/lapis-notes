...
| LN-PKG-063 | `@lapis-notes/ai` MUST live at `packages/plugins/plugin-ai`, expose build, check, test, and publint, and publish only its governed public surface. It MUST depend on API and Design Core, MUST NOT depend on `@lapis-notes/ui`, and MUST keep acpx and Codex types inside adapters.                                                                                                                                                                                                                                                           |
...
| LN-PKG-069 | Private `@lapis-notes/ai-host` MUST live at `packages/ai-host`, expose build, check, test, and a `lapis-ai-host` CLI. It MUST own acpx. `@lapis-notes/ai` MUST NOT depend on it at runtime.                                                                                                                                                                                                                                                                                         |
...
vault-scoped file mentions, Codex model catalog, chat settings, and movable chat panel.
...
Private `@lapis-notes/ai-host` owns acpx, process spawn, the WebSocket server,
and `lapis-ai-host serve`. Electron calls that library in-process. Web and
Storybook attach only with a configured URL and token.
