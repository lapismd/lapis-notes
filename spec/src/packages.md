...
| LN-PKG-014 | `@lapis-notes/api` `markupEditor` MUST accept a language id for `data-language` host attributes, default the editor face to Mira monospace with Markdown/text sans overrides, and mount the stock CodeMirror fold gutter for language-provided fold ranges when fold settings are enabled. |
