...
| LN-PKG-129 | Public Lapis Notes packages MUST start at version `0.1.0`, publish independently through the graph order UI, API, then Workspace, and keep internal runtime ranges compatible with the published provider version before a dependent package can be released. |
