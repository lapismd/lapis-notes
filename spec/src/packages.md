...
| LN-PKG-049 | `@lapis-notes/roles` MUST live at `packages/plugins/plugin-roles`, depend on `@lapis-notes/api` and `@lapismd/design-core`, and expose `build`, `check`, and `test`. It owns Roles workflows and retained CV behavior while delegating reusable form disclosure to public Design Core controls. |
