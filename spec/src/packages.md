...
| LN-PKG-134 | Lapis Notes manifests and its committed lockfile MUST consume extracted plugins through registry semver ranges and bind their API peer to the colocated framework package. A git-ignored pnpm hook MAY redirect only those ranges to checksum-verified sibling npm tarballs without changing host ranges or the lockfile; its stamp MUST distinguish a local `file:` package from the same registry version. Web, Deno desktop, Storybook, and AI integration development entrypoints MUST prepare that package-boundary install before launch. |
