...
| LN-PKG-120 | `@lapis-notes/graph` MUST own deterministic entrance layout, Obsidian-range camera, zoom-stable geometry, direct screen-pixel links, paint culling, exponential neighbourhood emphasis, reduced motion, semantic neutral and accent roles, and bounded layout telemetry. It MUST NOT add a renderer dependency outside its D3 force boundary. |
