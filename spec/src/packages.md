...
| LN-PKG-049 | `@lapis-notes/roles` MUST live at `packages/plugins/plugin-roles`, depend on `@lapis-notes/api` and `@lapismd/design-core`, and expose `build`, `check`, and `test`. It owns Roles workflows, retained CV behavior, and the ported legacy Applications presentation while delegating reusable leaf controls to public Design Core APIs. |
