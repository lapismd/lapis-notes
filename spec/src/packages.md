...
| ID         | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LN-PKG-001 | `@lapis-notes/api` MUST remain the shared runtime kernel: app container, vault/storage contracts, workspace model, plugins, commands, settings, metadata, and editor abstractions. Its public plugin dependency resolver MUST support host-owned namespace facades for declared renderer ESM shared dependencies and the exact compiler-emitted Svelte renderer ABI without bundling a second runtime copy into an installed plugin.                                                                                                                                                                                         |
...
| LN-PKG-012 | `@lapis-notes/api` MUST consume `@lapismd/mira` through its published npm package and built public exports for the shared source-editor CodeMirror shell. Its dependency and peer ranges MUST move together to the same reviewed Mira release line and remain portable semver ranges.                                                                                                                                                                                                                                                    |
...
| LN-PKG-036 | `@lapis-notes/api` MUST provide compact, interactive CodeMirror diagnostic hover cards and centered severity gutter markers. The card MUST remain stable while the pointer enters its controls. Rule links MUST retain readable foreground contrast and a persistent non-colour distinction. The implementation MUST consume public Design Core icons without importing Problems presentation internals, and its portaled action menu MUST keep the dropdown root and item primitives in the API-owned package context.                                      |
...
| LN-PKG-130 | The release workflow MUST keep Changesets version PR creation, build and retain a verified tarball manifest, stop with a manual-publish notice while an npm package is not registered, use npm trusted publishing with OIDC after bootstrap through the `npm-production` environment, and create package-scoped GitHub releases from the verified manifest. A partial manifest MAY omit published internal dependencies but MUST preserve dependency order among selected packages. All GitHub Actions jobs MUST use Node 24 and current Node 24-based action majors. |
...
| LN-PKG-134 | Lapis manifests and lockfile MUST consume extracted plugins through registry semver ranges and bind their API peer to the colocated framework. A git-ignored hook MAY redirect only those ranges to checksum-verified sibling tarballs without changing host ranges or lockfile; its stamp MUST distinguish a local `file:` package from the same registry version. Local preparation MUST preserve an existing hook so other source-package overrides remain composable. Web, desktop, Storybook, and AI development entrypoints MUST prepare that package-boundary install before launch. |
...
| LN-PKG-136 | `@lapis-notes/workspace` MAY consume public Design Core filter, forms, Resizable, and settings entrypoints for its private plugin-management presentation. It MUST NOT republish those primitives, add a second shared component API, or move registry and install policy into Design Core. |
| LN-PKG-137 | `@lapis-notes/workspace` MUST keep installed-versus-bundled registry actions, enablement action labels, and uninstall-or-disable confirmation policy private to plugin management while composing public Design Core Button, Field, Switch, Tooltip, and Alert Dialog primitives. |
| LN-PKG-138 | `@lapis-notes/api` MUST expose typed download-summary parsing and an optional distribution-manager accessor. Default registry sources MAY declare a statistics URL; Workspace MUST consume counts through that public boundary rather than fetching registry-owned files directly. |
...
LN-PLUG-087. Its public `PluginPlatform` type and registry schemas accept only
`web` and `desktop`; they do not retain an `electron` compatibility alias.
Workspace receives those contracts and owns the identity and gallery
presentation in LN-PLUG-088 without owning registry trust.
