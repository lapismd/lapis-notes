...
| LN-PKG-004 | Notebook and remaining unlisted plugin packages MUST NOT be added until specified and tracked in `MIGRATION.md`. The production hosts are Deno desktop and web. Authorized plugins are Markdown, Markdownlint, Spell Check, File Explorer, Bookmarks, Search, Graph, Bases, AI, History, Word Count, Roles, Tasks, Docs, and Terminal. Authorized host infrastructure MAY include sibling `@lapismd/ai-host` and sibling `@lapismd/terminal-host`.                                                                                                     |
...
| LN-PKG-043 | `@lapis-notes/file-explorer` MUST own the reusable File Explorer plugin implementation formerly co-located with Storybook. It MAY map Design Core current, reveal-or-create, and forced-new-tab activation intents to API workspace leaves and MUST persist Show hidden files, but MUST NOT own vault selection, session boot, persistence policy, or source-editor fixtures.                                                                                                                                                                          |
...
| LN-PKG-069 | Private `@lapismd/ai-host` MUST live in the sibling `ai-host` repository, version independently, and expose `build`, `check`, `test`, and a `lapis-ai-host` CLI. Lapis hosts MUST consume it through an explicit `link:` dependency or a `link:`-valued root override. `@lapis-notes/ai` MUST NOT depend on it at runtime.                                                                                                                                                                                                                             |
...
| LN-PKG-117 | `@lapis-notes/graph` MUST live at `packages/plugins/plugin-graph` as an enabled-by-default bundled plugin. It MUST expose its plugin, existing controls overlay, embed, renderer, views, settings, types, and styles through public package exports, depend on API and Design Core rather than `@lapis-notes/ui`, and retain runtime id `lapis-graph`. Package defaults MUST keep note nodes muted, load Global Graph with a centered whole-graph fit, and keep active-file focus an explicit zoom-and-center action. |
| LN-PKG-118 | `@lapis-notes/api` MUST expose backward-compatible indexed-metadata domain projections, ordered path-only Search matching, and the read-only MetadataCache reconciliation fingerprint. Memory, direct Turso, browser-coordinated, and desktop-native transports MUST retain one typed contract. |
| LN-PKG-119 | `@lapis-notes/graph` MUST own the canonical global coordinator, versioned AppDatabase snapshot, progress state, stale-while-revalidate behavior, and debounced settings persistence. It MUST keep canonical graph data independent from view lifetimes and share it across Global Graph leaves. |
| LN-PKG-120 | `@lapis-notes/graph` MUST own its deterministic entrance layout, Obsidian-range camera, zoom-stable geometry, paint culling, neighbourhood emphasis, reduced-motion behavior, semantic Graph colour roles, and bounded layout telemetry without adding a renderer dependency outside its existing D3 force boundary. |
| LN-PKG-121 | `@lapis-notes/graph` MUST compose public Design Core forms for Group colour and ordered rows, preserve `GraphGroupRule` exports, and own structured membership resolution plus chronological time-lapse. The package MUST NOT add a direct dependency on `@lapis-notes/search`. |
...
hosts register the plugin class and MUST NOT register Tasks views themselves. The sibling
`@lapis-notes/lapis-plugin-docs` package owns Core Docs notes, books, cards, and
FSRS review; desktop and web hosts register the plugin class and MUST NOT
register Docs views themselves. Other bundled
