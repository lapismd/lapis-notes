...
| LN-PKG-049 | `@lapis-notes/cv` MUST live at `packages/plugins/plugin-cv`, depend on `@lapis-notes/api` and `@lapismd/design-core`, and expose `build`, `check`, and `test`. It MUST own portable preview and PDF-export logic while composing shared form-header scrolling from Design Core. |
