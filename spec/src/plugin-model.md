...
| LN-PLUG-028 | Plugin and host-owned view registrations MUST remain attached to live `WorkspaceLeaf` instances when a pane-menu split duplicates their view. Split panes MUST NOT create a design-only tab or unprojected API leaf that bypasses the plugin view lifecycle. |
