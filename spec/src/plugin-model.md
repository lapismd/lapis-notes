...
| LN-PLUG-006 | The Lapis Notes default profile MUST contain exactly Source Editor, Markdown, File Explorer, and Search in that order. All four MUST default enabled and remain user-disableable. Extracted npm plugin constructors MUST resolve the same API peer instance used by the profile. Required host-owned views such as Problems MUST NOT become missing-view placeholders while their host registration remains. |
...
| LN-PLUG-043 | Both hosts MUST load configured installed plugins after static profile registration so registry and manual installs survive restart. Before evaluating a verified renderer ESM entry, the host MUST resolve every declared shared dependency plus the exact compiler-emitted `svelte`, `svelte/internal/client`, and `svelte/internal/disclose-version` ABI and expose each host-owned namespace without `globalThis.app` or a bundled runtime copy. Desktop Safe Mode, Workspace Trust, archive signature and hash verification, revocation handling, and duplicate-ID protection MUST remain enforced. |
