...
| LN-WS-007 | The shell MUST render design-core's default desktop and mobile chrome. It MUST NOT call the Lapis plugin loader, register an application `PluginProfile`, or install Lapis/community plugins. The design-core notifications presentation is the only required static shell plugin. F-Mode MAY be present as an optional static plugin. |
...
| LN-WS-016 | Bottom-panel layout changes MUST round-trip through the existing api-owned 1000 ms workspace writer. Design-core settings, including `appearence.interface.scrollbarVisibility`, MUST persist separately through api configuration with atomic writes and no layout/configuration feedback loop.                                                                                          |
...
| LN-WS-075 | Workspace status-bar projection MUST NOT emit `layout-change` or request a layout save when the visible status-item set is unchanged. An identical `upsertItem` MUST remain a no-op for layout persistence. |
...
| LN-WS-058 | Default desktop shells MUST show Design Core's built-in Settings action in the left ribbon footer while the ribbon is visible. The action MUST expose the public "Open settings" label and activate the same settings dialog as any shell settings trigger. |
| LN-WS-059 | When `loadLayout` finds no workspace file, the workspace MUST seed registered default sidebar views: left `file-explorer`, then `search`, then `bookmarks`; right `outline`, `file-properties`, then `tag`. It MUST open those docks, keep one empty main New Tab and a closed bottom dock, omit unregistered types, and MUST NOT write `workspace.json` during that seed. |
...
Default desktop chrome shows that Settings action in the left ribbon footer
