...
| LN-WS-013 | Workspace Storybook canvases MUST occupy the complete story viewport in desktop and mobile modes. Mobile mode MUST exercise the responsive shell across the available canvas rather than mounting an artificial fixed-width device card. Full-shell Autodocs embeds for `Workspace/Shell` and `Workspace/Lapis Editor Demo` MUST use the same shared isolated 700px story height as movable panel Docs, with Storybook shell padding removed so the application shell owns the documented viewport. Authored shell MDX MUST identify every scenario and render its canonical story description immediately before the corresponding canvas. |
