...
| LN-GOV-008 | `AGENTS.md` MUST identify the All Properties placement stories as the reference workflow for movable panels: one nested panel group covering the six real workspace surfaces unless a mapped requirement narrows scope, real-component Autodocs metadata with fixture-only kind/layout absent from args and property tables, minimal persisted shells, real grouped chrome, one real panel instance, stable `data-workspace-surface` host assertions and CSS-only top-level sidebar overrides over the body/bottom/grouped white default (never cached/runtime leaf-parent inspection), focused interactions, isolated 700px padding-free Autodocs canvases, explicit consumer-usable Show Code sources, and pending visual review. |
