...
| LN-GOV-026 | The spec-first map MUST route API storage to App Database, MetadataCache and registerIndexProjection to App Database and Plugin Model, and web changes to Web Host, Packages, and Architecture. Lapis Roles consumer stories MUST route to Roles Host Integration and Storybook Catalog.                                                                                                                                                                                                                                                                                                 |
...
| LN-GOV-033 | The spec-first map MUST route `packages/api/src/lib/storage/desktop-native.ts` to Desktop Host, Deno Desktop Host, and Packages. It MUST NOT treat that file as an App Database change.                                                                                                                                                                                                                                                                                                                                                                                                  |
| LN-GOV-034 | `AGENTS.md` MUST require published LapisMD packages to resolve through normal npm semver ranges, prohibit external workspace membership and vendoring, preserve portable publishable manifests, and route dependency defects to the owning repository before consumers update to released versions.                                                                                                                                                                                                                         |
| LN-GOV-035 | AI Host package implementation MUST be governed by the sibling `ai-host` repository's standalone specification. The Lapis spec-first map MAY keep a `packages/ai-host` cutover route to AI Plugin, Packages, Architecture, Desktop Host, and Web Host so the removed in-tree path stays classified.                                                                                                                                                                                                                                                                                      |
...
| LN-GOV-041 | The spec-first map MUST route Lapis package release workflows, Changesets configuration, release helper scripts, release documentation, and public package metadata to Packages and Specification Governance, with API, UI, and Workspace package docs also routed to their owning architectural chapters. |
...
command catalog. Its persisted Workspace inventory check requires the ten
audited enabled plugin IDs, including Graph, History, and Word Count, in both
the shell demo and the PersistedDesktop and Mobile stories.
...
published LapisMD dependency ranges rather than external workspace membership,
...
acceptance tests, icons, and entitlements remain reviewable inputs. Ignored
package-local desktop development symlinks for source imports are generated
bootstrap state, not tracked source. The change map requires desktop
implementation slices to update the desktop, package, and architecture chapters;
language-runtime changes additionally update Problems.
