...
| LN-CAT-022 | `Workspace/Panels/Markdown/All Properties` MUST group the movable-surface spike as six focused child stories rather than adding them to the flat Markdown panel list: Middle (Top Tabs), Stacked Tabs, Left Sidebar, Right Sidebar, Bottom Panel (using grouped-panel chrome), and Sidebar As A Group. The stories MUST declare the exported real `AllProperties` component as their Autodocs authority, showing only its `app: App` input; `PanelDemo` kind/layout fixture selection MUST be fixed in story render functions or parameters and absent from args, Controls, and Properties. The stories MUST use the real minimal Lapis shell, keep the panel as the visual and interaction focus, carry literal `visual-pending` tags, and ship independent nested-import baselines. Each play MUST assert the stable design-core destination host and nested `WorkspaceViewHost`, then prove the panel and sticky chrome match the host's resolved paint: body, bottom panel, and sidebar group retain the white workspace default, while only ungrouped top-level left/right sidebar view hosts receive panel paint. Each app-backed Autodocs canvas MUST render in an isolated iframe at exactly 700px high so simultaneous examples retain their own app and placement. The panel-specific Docs canvas MUST remove `.docs-story .sb-story` padding so the shell reaches the preview edges. Each Show Code panel MUST use an explicit, consumer-usable source snippet derived from the corresponding persisted layout; it MUST show public Lapis imports and the real placement state, not `PanelDemo` or inferred story args. |
