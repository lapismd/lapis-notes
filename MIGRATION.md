...
- [x] File Properties → Mira `FrontmatterEditor` + Lapis `MetadataTypeManager` adapter, 15rem accessible minimum plus narrow single-column rows, native focus geometry with view-token contrast fill, hash Tags icon, and surface-contrasting native pills (LN-MD-017/019)
