...
| `@lapis-notes/desktop-deno`                | In progress         | Deno 2.9.5+ `deno desktop` parity track: shared vault/shell renderer, WASM Turso, first-party plugins, packaged native Markdown diagnostics, canonical-path `Deno.watchFs` delivery, native services, lifecycle, and local distribution. MUST NOT replace Electron before equivalent packaged acceptance.                     |
