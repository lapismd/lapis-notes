...
| `@lapis-notes/ai`                           | In progress    | Bundled provider-agnostic AgentRuntime, host-gated live adapters on `./runtimes`, Codex and Cursor via ACP, Fake without a host, and a Design Core chat panel; CV-domain MCP tools remain outside this package                                      |
| `@lapis-notes/ai-host`                      | In progress    | Private process host: acpx executor, `lapis-ai-host serve`, required WebSocket token handshake; Electron uses the library in-process; PWA attach and Storybook `Plugins/AI/Live Host` use URL and token |
