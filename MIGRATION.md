...
| `@lapis-notes/cv`                           | Done (CV slice) | Canonical legacy normalization, lossless structured editing, recoverable invalid YAML, serialized autosave, RenderCV-aligned pane-relative preview zoom, browser/vault PDF export, full-width borderless Lapis-themed Mira Markdown modes, disclosure-safe grouped main-toolbar document actions, constrained form-header scrolling, CV-only theme shortcuts, and governed 7020 Storybook; Applications remain later and Search does not index `*.cv.yml` |
