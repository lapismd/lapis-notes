...
| `@lapis-notes/cv`                           | Done (CV slice) | Canonical legacy normalization, lossless structured editing, recoverable invalid YAML, serialized autosave, browser WASM preview, Lapis-themed Mira Markdown Preview/read-only Source modes, and governed 7020 Storybook; Applications remain later and Search does not index `*.cv.yml` |
| Notebook / remaining unlisted plugins        | Not started    | Remain blocked by LN-PKG-004 until separately specified                                                                                                                                                                              |
| `@lapis-notes/file-explorer`                | Done (partial) | Reusable File Explorer plugin shared by Storybook and Electron; single-click reuse and forced modifier-open map to API workspace leaves, while distinct double-click reuse-or-create behavior remains pending under LN-ED-047; source-editor remains a Storybook-local fixture |
