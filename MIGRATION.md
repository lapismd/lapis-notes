...
| `@lapis-notes/roles`                        | In progress (parity correction) | Clean rename and vault workflows are complete. Applications, Activity, Actions, and role detail are being corrected to the exact legacy CV component/CSS presentation after the replacement composition was rejected. Tasks and AI remain excluded; visual baseline work is deferred at user request. |
